...
try {
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        $queryname = "SELECT * FROM category WHERE name like '%$category_name%' ";
        $get_category_name = $categorydb->conn->prepare($queryname);
        $get_category_name->execute();
        $query_result = $get_category_name->rowCount();
        if($query_result > 0){
            while ($row = $get_category_name->fetch(PDO::FETCH_ASSOC)){
                extract($row);
                ...
                array_push($response["output"], $categorynamedata);
            }
            // set error schema
